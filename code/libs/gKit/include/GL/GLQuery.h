...
inline std::string OverlapGPUSummary( const std::string &message, const GLCounterState& a, const GLCounterState& b )
{
    char tmp[1024];
...
inline std::string OverlapCPUSummary( const std::string &message, const GLCounterState& a, const GLCounterState& b )
{
    char tmp[1024];
...
public:
    ...
    {
        //~ GLint64 start; glGetInteger64v(GL_TIMESTAMP, &start);
        //~ GLuint count; glGetQueryObjectuiv(name, GL_QUERY_RESULT, &count);
        //~ GLint64 stop; glGetInteger64v(GL_TIMESTAMP, &stop);
        //~ GLint64 time= (stop - start) / 1000;
        //~ printf("query result sync: %luus\n", time);
    ...
    GLuint64 result64( )
    {
        ...
        //~ GLint64 stop; glGetInteger64v(GL_TIMESTAMP, &stop);
        //~ GLint64 time= (stop - start) / 1000;
        //~ printf("query result64 sync: %luus\n", time);
